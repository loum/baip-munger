PY=/usr/bin/python
NOSE=/usr/bin/nosetests1.1 -s -v --with-xunit --with-coverage --cover-erase --cover-package baip_munger
NOSE_ENV=.env/bin/nosetests -s -v --with-xunit --with-coverage --cover-erase --cover-package baip_munger
GIT=/usr/bin/git
...
TEST=baip_munger.tests:TestMunger \
	baip_munger.tests:TestXpathGen
...
uninstall:
	$(RPM) -e python-baip-munger

install:
	$(RPM) -ivh dist/python-baip-munger-?.?.?-?.noarch.rpm

upgrade:
	$(RPM) -Uvh dist/python-baip-munger-?.?.?-?.noarch.rpm
